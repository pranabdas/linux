...
The first line, starting with shebang (`#!`), indicates which program it should
use to run the script.

Another, perhaps better alternate to above shebang line is:
```bash
#!/usr/bin/env bash
```

The advantage of second option is that it will search for the program (in this
case bash) based on the `PATH` environment variable. This is also useful if the
`PATH` variable on a system has been configured to point to an alternate version
of the program. For instance, one might install a newer version of bash while
preserving the original version and insert the location of the newer version
into the `PATH` variable. The use of #!/bin/bash would result in using the
original bash, while #!/usr/bin/env bash would make use of the newer version.
...
Alternatively, we can use shell glob instead of `ls`, which runs in a sub-shell
and is less efficient:
```bash
for file in *; do mv ${file} slide_${file#item_*}; done; ls

for file in *.md; do mv ${file} slide_${file#item_*}; done; ls
```

Or, we can use `find` command to find the files:
```bash
...
- https://google.github.io/styleguide/shellguide
